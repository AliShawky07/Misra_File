...
static void HandShakingStateMachine(void)
{
	...
	switch (BLMGR_HandShakeState)
	{
		...
		case (u8)HANDSHAKE_STATE_SEND_ID_FRAME:
		{
			...
			BLTD_StartReceivingData( BLMGR_DataRxBuffer, ID_FRAME_LENGTH, sRxBufferDnCallBackNotif);
...
void BLMGR_SetDeviceName(u8 DeviceName[],u8 DeviceNameLength)
{
	...
	BLMGR_TxDeviceNameLength =(u8) DeviceNameLength;
}
/*********************************************************************************/
...
static u8 CheckIdFrame(void)
{
	...
	if (TempVar == (u8)BLMGR_DataRxBuffer[FRAME_CHECKSUM_IDX])
	{
...
{
	u8 ErrorType;
	ErrorType = (u8)BLMGR_DataRxBuffer[ERROR_TYPE_IDX];
	switch(ErrorType)
	{
...
{
	u16 LoopIndex;
	for(LoopIndex = (u8)0; LoopIndex <(u8) Length; LoopIndex ++)
	{
